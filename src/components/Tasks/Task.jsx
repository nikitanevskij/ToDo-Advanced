...
export default function Task({ item, onRemove, list, onEdit, onCompleteTask }) {
  const onChangeCheckBox = (e) => {
    onCompleteTask(list.id, item.id, e.target.checked);
  };
  ...
  return (
    <div key={item.id} className="tasks__items-row">
      ...
      <div className="tasks__items-row-actions">
        <div onClick={() => onEdit(list.id, { id: item.id, text: item.text })}>
          <svg
            width="15"
            height="15"
            viewBox="0 0 15 15"
        ...
        <div onClick={() => onRemove(list.id, item.id)}>
          <svg
            width="11"
            height="11"
            viewBox="0 0 11 11"
